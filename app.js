...
// IMG-009: 'threshold' (applyThreshold(), the pre-IMG-009 only behavior) or 'subject'
...
el('exportProdSheetSVG').onclick=()=>{if(!layout){el('status').textContent='Export failed: layout is not ready yet.';return}try{updateProdSheetReadabilityValidation();download('rhinestone-production-sheet.svg','image/svg+xml',productionSheetToSvg(layout,currentProductionSheetOptions()))}catch(error){el('status').textContent=`Export failed: ${error.message}`;el('prodSheetValidation').textContent=`Export failed: ${error.message}`}};
el('exportProdSheetPDF').onclick=()=>{if(!layout){el('status').textContent='Export failed: layout is not ready yet.';return}try{updateProdSheetReadabilityValidation();download('rhinestone-production-sheet.pdf','application/pdf',productionSheetToPdf(layout,currentProductionSheetOptions()))}catch(error){el('status').textContent=`Export failed: ${error.message}`;el('prodSheetValidation').textContent=`Export failed: ${error.message}`}};
...
}catch(error){el('status').textContent=`Export failed: ${error.message}`;el('prodSheetValidation').textContent=`Export failed: ${error.message}`}};
