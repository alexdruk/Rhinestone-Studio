...
## Glyph Separation (READ-006)

`GlyphSeparation.js` holds the letter-crowding primitives shared by the font-certification tooling
(`tools/font-certification/`) and the product's **Separate letters** control. It generates no
geometry — it measures a `StoneLayout` the engine already produced, or resolves per-character
outlines through `resolveTextPolygons()`:

* `countClusters(stones, thresholdMm)` — connected-cluster count of a stone list (union-find).
* `overlapComponentCount(polygons)` / `expectedComponentCount(engine, fontId, text, providerId)` —
  the signal-F denominator: the number of geometrically-connected blobs a word *should* have,
  measured per character and summed. Deliberately not `groupPolygonsIntoComponents()` — see the
  module's own doc comment. Memoized per `(providerId, fontId, character)`; returns `null` for an
  authored stone font (no vector outline).
* `separationBand(ratio)`, `SEPARATION_TARGET`, `CLUSTER_GAP_MULTIPLIER`, `TRACKING_XPITCH_LADDER` —
  the constants the READ-005a tracking experiment validated.
* `solveLetterSpacingMm({ engine, layerParams, pitchMm })` — a linear ascending scan of
  `TRACKING_XPITCH_LADDER` (never bisected — `separationRatio` is not monotone in `letterSpacingMm`)
  that returns the moment `countClusters / expectedComponentCount` reaches `SEPARATION_TARGET`; the
  ladder is ascending, so the first hit is the lowest rung and rungs above it don't matter. Returns
  `{ letterSpacingMm, xPitch, separationRatio, separationAchieved: true, widthMm, untrackedWidthMm }`
  or `{ separationAchieved: false, untrackedWidthMm }`. Only a genuine failure evaluates all eleven
  rungs; it never falls back to the top rung.

Design, evidence, and the `app.js` wiring: `docs/specifications/READ-006-LetterSpacing.md`.
