/**
 * RS-0003.5E1 / RS-2001 — shared bridge for the `.rhs` example fixture schema.
 *
 ...
 * reasoning. `toAppProjectShape()` below bridges the two; it does not change either live schema.
 *
 * RS-2001: this module moved here (from tools/lib/rhsProject.mjs) so it has exactly one
 * implementation shared by both the Node-side fixture regression/benchmark suite
 * (tools/test-examples-regression.mjs, tools/test-gallery-benchmark.mjs, via the
 * tools/lib/rhsProject.mjs re-export shim) and the browser Gallery (src/gallery/index.js),
 * which uses toAppProjectShape() to open a read-only example as a live, editable copy through the
 * app's own existing validateProject()/project-import path. Nothing here invents a stone
 * position, a project schema, or a rendering pipeline.
 *
...
export const SUPPORTED_IMAGE_FILL_MODES = new Set(['fill', 'staggered', 'radial', 'contour', 'organic']);
...
// RS-2001: the two preserved legacy fixtures (vitalina.rhs/vitalina-serbin.rhs, byte-checksum-
// locked -- see tools/test-examples-regression.mjs check 2) store `color` as the display name
// "Crystal AB" rather than the crystal color catalog's id "crystal". That was harmless while this
// schema was only ever consumed by Node-side tooling (Stone.color is stored/rendered as an opaque
// string either way), but the Gallery is the first feature to open a `.rhs` fixture into the
// *live* editor's own #stoneColor <select>, whose <option value> list is real catalog ids only --
// setting an unmatched value silently collapses the control to "", and the next tracked-field edit
// then writes that empty string back as the layer's color, breaking future geometry generation.
// Since the fixture files themselves cannot change (byte-locked), this bridges legacy display
// names to their real id here, exactly like resolveFontId() above bridges legacy font family
// names -- unrecognized/already-valid values pass through unchanged, never thrown on, since color
// is decorative, not a hard requirement.
const STONE_COLOR_NAME_TO_ID = Object.fromEntries(Object.values(STONE_COLORS).map((c) => [c.name, c.id]));
export function resolveStoneColorId(colorValue) {
  if (typeof colorValue !== 'string') return colorValue;
  if (STONE_COLORS[colorValue]) return colorValue;
  return STONE_COLOR_NAME_TO_ID[colorValue] ?? colorValue;
}
...
/**
 * Translates a validated `.rhs` project to app.js's ad hoc live-editor schema. Used by the
 * regression suite (to cross-check app.js's real validateProject()) and by the browser Gallery
 * (src/gallery/index.js) to open a read-only example as a fresh, editable project — app.js itself
 * is not modified by this translation.
 */
    ...
    layers: rhsProject.layers.map((layer) => {
      ...
      if (layer.type === 'text') {
        return {
          ...
          gap: layer.gapMm,
          color: resolveStoneColorId(layer.color),
 ...
 * @param {import('../geometry/GeometryEngine.js').GeometryEngine} permanentEngine
