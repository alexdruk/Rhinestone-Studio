...
import { AI_STONE_PALETTE_CAP, weightedLabDistance, chooseAiStonePalette } from './WeightedLabPalette.js';
...
// IMG-024 (D4): the most subject pixels the error rule's greedy palette choice looks at.
export const ERROR_PALETTE_MAX_SAMPLES = 40000;

/**
 * IMG-024 (D4): the 'error' palette rule. Chooses up to `maxColors` catalog colours by
 * chooseAiStonePalette()'s greedy (minimum summed weighted-Lab error) over a fixed-stride sample of
 * the eligible pixels, then labels every eligible pixel with its weighted-Lab-nearest pick (ties to
 * the lower palette index). A pick left with no pixels is dropped from `keptIds`; `pickCount` still
 * counts it. See docs/specifications/IMG-024-FlatArtworkStyle.md.
 *
 * @returns {{labels: Uint8Array, keptIds: number[], pickCount: number, finalCounts: number[], eligibleCount: number}}
 */
export function labelErrorPaletteColors({ r, g, b, eligible, palette, catalogLabs = palette.map((entry) => rgbToLab(...hexToRgb(entry.hex))), maxColors = AI_STONE_PALETTE_CAP, chromaScale = 1 }) {
  const pixelCount = eligible.length;
  const labels = new Uint8Array(pixelCount).fill(255);
  const finalCounts = new Array(catalogLabs.length).fill(0);
  const labs = [];
  ...
  const eligibleCount = labs.length;
  if (eligibleCount === 0) return { labels, keptIds: [], pickCount: 0, finalCounts, eligibleCount };

  const stride = Math.max(1, Math.ceil(eligibleCount / ERROR_PALETTE_MAX_SAMPLES));
  const sample = [];
  for (let k = 0; k < eligibleCount; k += stride) sample.push(labs[k]);
  const picks = chooseAiStonePalette(sample, catalogLabs, maxColors);

  for (let k = 0; k < eligibleCount; k++) {
    let best = picks[0], bestD = Infinity;
    for (const c of picks) {
      const d = weightedLabDistance(labs[k], catalogLabs[c]);
      if (d < bestD) { bestD = d; best = c; }
    }
    labels[indices[k]] = best;
    finalCounts[best]++;
  }
  const keptIds = picks.filter((c) => finalCounts[c] > 0);
  return { labels, keptIds, pickCount: picks.length, finalCounts, eligibleCount };
}
 ...
 * @param {string|null} [args.paletteRule] IMG-024: exactly 'error' labels with
 *   labelErrorPaletteColors() instead; anything else is the labelCatalogColors() path unchanged.
 * @returns {{labels: Uint8ClampedArray, colorGroups: {rgb: number[], pixelShare: number, nearestId: string}[]}}
 */
export function quantizeColors({ r, g, b, data, colorCount, palette, chromaScale = 1, paletteRule = null }) {
  ...
  const { labels: catalogLabels, keptIds, finalCounts, eligibleCount } = paletteRule === 'error'
    ? labelErrorPaletteColors({ r, g, b, eligible, palette, maxColors: colorCount, chromaScale })
    : labelCatalogColors({ r, g, b, eligible, palette, maxColors: colorCount, chromaScale });
  if (eligibleCount === 0) {
