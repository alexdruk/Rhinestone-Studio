...
export function createDrawingTool(canvasEl, hooks = {}) {
   ...
   * layer only.) The silence is gone too: Trace/Eraser now fire onTraceRejected/onEraseRejected
   * with a reason, and Stamp's onStampPlace already messages a null/non-'path' resolution.
   *
   * This bare-id form is Stamp's (resolveStampTargetLayerId delegates straight to it, and
   * onStampPlace is always called with layerId possibly null); Trace/Eraser call
   * resolveMarkTargetByBounds() directly for the {layerId, blockedByIneligible} miss reason.
   *
   ...
   * needs the looser test. Stamp keeps the bare-id form (onStampPlace is always called, layerId
   * may be null, and app.js's onStampPlace already messages both the null and non-'path' cases);
   * Trace/Eraser use resolveMarkTargetByBounds() directly so they can plumb the miss reason.
   * @param {paper.Point} point
   * @returns {string|null}
   */
