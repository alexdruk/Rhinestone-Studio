...
**Implementation status:** one project model exists. `app.js` builds and edits its own ad hoc
plain-object project (`defaultProject()`/`validateProject()` in `app.js`), mm-only, no pixels —
consumed identically by Gallery, Design Library, Save/Load, Production Sheet, Import/Export, and
History. A second, fully-built but wholly unused parallel model (`src/core/Project.js`/`Layer.js`)
existed from RS-0003.5B3 through RS-2005 and was removed by RS-2006 (Project Model Consolidation)
once the audit confirmed nothing in the live app ever imported it — see
`docs/specifications/RS-2006-ProjectModelConsolidation.md`.
...
**Implementation status:** not implemented as a dedicated module. `app.js`'s own
`validateProject()` checks canvas dimensions, units, and layer shape on import, but there is no
duplicate-layer-id check, no overlap detection, and no missing-font validation anywhere in the
codebase; a missing font manifest is instead handled as a runtime error surfaced in the status bar
(see "Current Architectural Limitations"). A prior, separate `validate()` implementation lived on
the now-removed unused `src/core/Project.js` model (RS-2006) and was never reachable from the live
app either, so its removal changes nothing about this gap.
...
**Implementation status:** `app.js`'s live project object is the one layer model in the
repository, supporting six editable layer types (text/circle/rectangle/svg/image/path) as plain
objects. A parallel, never-imported `src/core/Layer.js` (`TextLayer`/`CircleLayer`/`RectangleLayer`
plus generic `svg`/`manual` slots) existed alongside it from RS-0003.5B3 through RS-2005 and was
removed by RS-2006 once confirmed unreachable from the live app — see
`docs/specifications/RS-2006-ProjectModelConsolidation.md`. As of RS-1001, an `svg` layer
...
saved before RS-1009 (no `x`/`y` on its text layers) renders byte-identical to before.
...
| Organic Fill Bridson Poisson-disk sampling (IMG-003, generalized IMG-004) | `src/geometry/OrganicSampler.js` | nothing else in `src/**` (a field-agnostic Poisson-disk point sampler taking a plain `insideAt()` interior-test callback, with its own private `mulberry32` PRNG copy; `src/geometry/StoneSampler.js` is its only caller — see `docs/specifications/IMG-003-OrganicPlacement.md`). IMG-004 generalized `samplePoissonDiskPoints()` with an optional per-point `radiusAt()`/`maxRadiusMm` variable-radius mode, called by `StoneSampler.js`'s new `sampleEdgeFieldFillPoints()`; `StoneSampler.js` remains its only caller — see `docs/specifications/IMG-004-EdgeAwareness.md` |
| Edge detection for edge-aware placement (IMG-004) | `src/image/Edge.js` | nothing else in `src/**` (a pure Sobel-gradient + monotonic-deque max-filter field transform, `edgeChannel(field, bandRadiusPx)`; no `src/geometry` import, mirroring `src/image/Blur.js`; `src/image/ImageFieldPipeline.js`'s `prepareImageField()` is its only caller — see `docs/specifications/IMG-004-EdgeAwareness.md`) |
...
| Bitmap image trace (RS-1008, corrected RS-1008A, extended IMG-001, IMG-004) | `src/image/**` | nothing else in `src/**` (pure field-preparation only: grayscale/threshold/invert/optional transparency mask/blur/resize/edge-detect -> a neutral multi-channel field `{data, luminance, alpha, edge, labels}`; zero dependency on `src/geometry/**`, mirroring `src/svg/**`) |
...
Every permanent module (`src/fonts`, `src/text`, `src/geometry`, `src/renderer`,
...
flowchart TD
    subgraph AppState["app.js — orchestration"]
        Project["app.js project object\n(the one canonical project model)"]
...
to never import an internal file of another module directly.

`app.js` owns:

- **The project/layer state** — `defaultProject()` and all layer mutation functions
  (`writeSelectedControlsToLayer()`, `duplicateLayer()`, `deleteLayer()`, drag/resize handlers).
  This is the one canonical project/layer model in the repository (RS-2006 removed the previously
  unused parallel `src/core/Project.js`/`Layer.js` implementation — see "Remaining Legacy / Dead
  Code").
...
**RS-2006 (Project Model Consolidation) deleted `src/core/Project.js`/`Layer.js`/`index.js`** (and
their dedicated `tools/test-core-model.mjs`), the fully-built but never-imported parallel
project/layer model that had existed since RS-0003.5B3. A repository-wide audit confirmed it had
zero real references anywhere (`app.js`, every `src/**` module, and every `tools/**` test) — the
live app's own ad hoc project/layer schema was already the one model every subsystem (Gallery,
Design Library, Save/Load, Production Sheet, Import/Export, History) consumed. See
`docs/specifications/RS-2006-ProjectModelConsolidation.md`.
...
1. **Cross-layer merge lives outside the Geometry Engine.** The proximity `dedupe()` that merges
...
5. **`getBrowserDependencyProbeStatus()` is unused.** The function exists but nothing calls it;
   only the module's side effect (import-time resolution) is exercised, by both `app.js` and its
   guard test.
6. **`svg` layers use a differently-named Fill Style field than every other vector layer type.**
