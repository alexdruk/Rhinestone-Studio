...
| Organic Fill Poisson-disk geometry (IMG-003) — **specified, not yet implemented** | `src/geometry/OrganicSampler.js` | nothing else in `src/**` (planned: a Bridson Poisson-disk point generator taking a plain interior-test callback, the same shape `ContourRingSampler.js` takes above; `src/geometry/StoneSampler.js` would be its only caller — see `docs/specifications/IMG-003-OrganicPlacement.md`). This row is a forward reference, unlike every other row in this table — `src/geometry/OrganicSampler.js` does not exist in the repository as of this document's synchronization commit; reword to a normal row (or confirm unchanged) once it lands. |
