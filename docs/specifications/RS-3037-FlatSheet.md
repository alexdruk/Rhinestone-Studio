...
| `app.js:5360-5361` (`#exportCup`/`#exportCombined` `.onclick`) — capture `cupCanvas` directly, or via `composeCombinedPreviewCanvas()` (which itself reads `cupCanvas`), and download the result | Not template-aware; assumes `cupCanvas` was just redrawn by `drawCup()` | **Follow-up fix (found after the `drawCup()` no-op above landed)**: since `drawCup()` no longer redraws `#cup` while sheet is active, both buttons would otherwise export a stale image of whatever template was active before, or a blank canvas on a fresh sheet project. Each handler gets a leading guard, before the existing `if(!layout)` guard: `if(currentObjectTemplate().id==='sheet'){el('status').textContent='Flat Sheet has no 3D preview to export.';return}`. Both buttons are also hidden (`style.display=isSheet?'none':''`, `''` so the show-branch hands display back to `button.btn`'s own stylesheet `display:inline-flex` rather than overriding it — see the 2nd follow-up row below) alongside the other `isSheet` toggles in `updateObjectTemplateDetail()` (next row) as a discoverability measure, not a substitute for the handler guard (either button remains reachable via keyboard/devtools/a stale DOM reference). |
| `app.js:6189-6194,6249` (all four `#exportCup`/`#exportCombined`/`#imageStudioSwitchToSheet` `style.display` show-branches above) | **2nd follow-up fix (visual regression)**: the first pass above used `'block'`/`'inline-block'` as the show-value. `button.btn` (`index.html:69`) is `display:inline-flex` in the stylesheet — an explicit `'block'`/`'inline-block'` inline style overrides that and visibly changes these buttons' layout (icon/label alignment) on every non-sheet project, not just while sheet is active. | Every show-branch of these three toggles now uses `''` instead: `el('exportCup').style.display=isSheet?'none':''`, `el('exportCombined').style.display=isSheet?'none':''`, and both `#imageStudioSwitchToSheet` sites (`updateObjectTemplateDetail()`/`renderImageStudio()`) `...?'none':''`. `''` clears the inline override so the stylesheet's own `display:inline-flex` governs the shown state, identical to every untouched `button.btn` elsewhere. `#sheetFields`' `'block'` (a `<div class="field-section">`, not a `button.btn`) is unaffected and unchanged — it mirrors `#plateFields`'s own already-correct `'block'`. |
...
13. The Image panel's "Switch to Flat Sheet" button: its real extracted `onclick` handler, run
    ...
    is active, `''` otherwise (hands display back to `button.btn`'s own stylesheet rule — see item 14's
    2nd-follow-up note).
14. **Follow-up: `#exportCup`/`#exportCombined` no-op on sheet.** `drawCup()`'s no-op for sheet
    ...
    `''` otherwise — see the 2nd follow-up row in §A.2) in `updateObjectTemplateDetail()`.
