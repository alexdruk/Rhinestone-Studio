...
from paths import SOURCE_FONT, CONFIG_DIR, output_dir, repo_relative
from lib.font_build import generate_variant

ALL_SIZES = ["SS6", "SS10", "SS16", "SS20", "SS30"]


def resolve_config(size_id):
    config_path = CONFIG_DIR / f"{size_id}.json"
    with open(config_path) as f:
        raw = json.load(f)

    source_font = TTFont(str(SOURCE_FONT))
...
def generate_one(size_id, verbose=True):
    config = resolve_config(size_id)
    out_dir = output_dir(size_id)
    out_font_path = out_dir / f"SacramentoRhinestone_{size_id}.ttf"
    metadata_path = out_dir / f"generation-metadata.{size_id}.json"
